...
| `Mcv2Bench.java key=value...` | the encoder benchmark behind the report's encode times: encodes a raw RGB source with a profile (or a custom live search), leaves out the warm-up frames, and prints one JSON line - mean/p50/p95/max ms per frame, CPU and bytes allocated per frame, keyframes, map and zlib rates, PSNR. Compile with a JDK against mcav-common, run on the JVM being measured (the report: Temurin 25); its Javadoc has the arguments and an example |
