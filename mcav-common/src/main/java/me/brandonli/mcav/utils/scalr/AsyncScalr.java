 ...
 */
package me.brandonli.mcav.utils.scalr;
...
import me.brandonli.mcav.utils.scalr.Scalr.Method;
import me.brandonli.mcav.utils.scalr.Scalr.Mode;
import me.brandonli.mcav.utils.scalr.Scalr.Rotation;
...
public class AsyncScalr {
  ...
  static {
    if (THREAD_COUNT < 1) {
      throw new AssertionError(
        "System property '" + THREAD_COUNT_PROPERTY_NAME + "' set THREAD_COUNT to " + THREAD_COUNT + ", but THREAD_COUNT must be > 0."
      );
    }
  }
   ...
   */
  public static Future<BufferedImage> crop(final BufferedImage src, final int width, final int height, final BufferedImageOp... ops)
    throws IllegalArgumentException, ImagingOpException {
    checkService();
   ...
   */
  public static Future<BufferedImage> pad(final BufferedImage src, final int padding, final Color color, final BufferedImageOp... ops)
    throws IllegalArgumentException, ImagingOpException {
    checkService();
   ...
   */
  public static Future<BufferedImage> resize(final BufferedImage src, final int targetSize, final BufferedImageOp... ops)
    throws IllegalArgumentException, ImagingOpException {
    checkService();
   ...
   */
  public static Future<BufferedImage> rotate(final BufferedImage src, final Rotation rotation, final BufferedImageOp... ops)
    throws IllegalArgumentException, ImagingOpException {
    checkService();
  ...
  protected static ExecutorService createService(final ThreadFactory factory) throws IllegalArgumentException {
    if (factory == null) {
  ...
  protected static class DefaultThreadFactory implements ThreadFactory {
    ...
    DefaultThreadFactory() {
       ...
       * in.
       */
      this.group = (manager == null ? requireNonNull(Thread.currentThread().getThreadGroup()) : manager.getThreadGroup());
